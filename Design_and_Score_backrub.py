...
directory_name = "/ifs/scratch/home/bs3281/TF_Project_BS/Tests/"
run_name = "BS_test_original_br_protocol"
...
print(f"Rosetta Design:        {USE_RosettaDesign}")
...
print(f"Num backrub structures:{TIED_FLAG}\n")
...
# ============================================================
# pyrosetta.init(
#     extra_options="-mute basic -mute core -mute protocols "
#                   "-relax:default_repeats 1 -ignore_zero_occupancy false" 
# )
pyrosetta.init(
    extra_options="-relax:default_repeats 1 -ignore_zero_occupancy false" 
)
...
def execute_pipeline(
    pdb_paths: list,
    protein_mpnn_1_path: str,
    protein_mpnn_2_path: str,
    backrub_path: str,
    temp_dir: str,        # ⭐ ADD THIS
    n_thread: int,
):
    ...
    try:
        ...
        print(f"[WARNING] Could not set CPU affinity for thread {n_thread}: {e}")
    
    if not pdb_paths:
        print(f"[INFO] Thread {n_thread} received no PDBs — skipping.")
        return []
    ...
    print(f"[INFO.execute_pipeline] pdb_paths: {pdb_paths}")

    # Backrub stage
    if USE_backrub:
        print(f"[INFO.execute_pipeline] USE BACKRUB TRUE")
        backrub_designs = energy_methods_original.perform_chainA_backrub(
            pdb_paths,
            backrub_path,
            iterations=backrub_iterations,
    ...
    print("[INFO.execute_pipeline] Step 1 passed:", step_1_passed)
    ...
    for file in step_1_passed:
        ...
        try:
            ...
            print(f"[ERROR.execute_pipeline] Step 1 failed for {file}: {e}")
            continue
    ...
    print("[INFO.execute_pipeline] Step 2 passed:", step_2_passed)
    ...
    for file in step_2_passed:
        ...
        try:
            ...
            print(f"[ERROR.execute_pipeline] Step 2 failed for {file}: {e}")
            continue
...
def main():
    ...
    temp_dir = run_dir / "temp_files"
    step1_dir = run_dir / "step1"
    step2_dir = run_dir / "step2"
    backrub_dir = run_dir / "backrub_outputs"

    for d in [base_output, run_dir, scoring_dir, temp_dir, step1_dir, step2_dir, backrub_dir]:
        make_dir(d)
    ...
    for thread_num in range(num_threads):
        ...
        p = multiprocessing.Process(target=execute_pipeline, args=args)
